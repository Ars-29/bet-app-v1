...
export async function findPlayerWithGemini(matchDetails, participantName) {
    ...
    if (!geminiApiKey) {
        console.log(`   ⚠️ GEMINI_API_KEY not found in environment, skipping Gemini fallback`);
        return null;
    }
    
    try {
        console.log(`   🤖 Using Gemini AI fallback to find player: "${participantName}"`);
...
Example: If player name is "K. Etta" and list has "Karl Etta" with ID 1234567, return: 1234567`;
        ...
        if (playerIdMatch) {
            ...
            if (matchedPlayer) {
                console.log(`   ✅ Gemini matched: "${matchedPlayer.name}" (ID: ${matchedPlayerId})`);
        ...
        // Don't throw - just return null so normal flow continues
        return null;
    }
}
